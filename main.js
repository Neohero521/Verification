// 【Verification主业务逻辑】严格对齐Cola仓库main.js实现
...
function onButtonClick() {
    toastr.info(`The checkbox is ${extension_settings[extensionName].example_setting ? "checked": "not checked"}`, "Verification插件");
}

// 页面可见性监听，对齐Cola的异常重置逻辑
...
async function loadSettings() {
    console.log('[Verification] 开始加载配置');
    ...
    console.log('[Verification] 配置加载完成，UI渲染完成');
}

// 事件绑定，对齐Cola的事件管理
function bindAllEvents() {
    console.log('[Verification] 开始绑定全局事件');
    // 示例配置
    ...
    $("#parse-chapter-btn").off("click").on("click", () => {
        ...
        if (!file) {
            toastr.warning('请先选择小说TXT文件', "Verification插件");
            return;
        }
        if (customRegex) {
        ...
        reader.onload = (e) => {
            ...
            if (customRegex) {
                ...
                toastr.info(`正在使用【${regexName}】解析，匹配到${currentRegexItem.count}个章节`, "Verification插件");
            }
        ...
        reader.onerror = () => toastr.error('文件读取失败，请检查文件编码（仅支持UTF-8）', "Verification插件");
    ...
    $("#split-by-word-btn").off("click").on("click", () => {
        ...
        if (wordCount < 1000 || wordCount > 10000) {
            toastr.error('单章字数必须在1000-10000之间', "Verification插件");
            return;
        }
        const reader = new FileReader();
        reader.onload = (e) => {
    ...
    $("#stop-send-btn").off("click").on("click", () => {
        if (state.isSending) {
            ...
            toastr.info('已停止发送', "Verification插件");
        }
    });
    ...
    $("#graph-stop-btn").off("click").on("click", () => {
        if (state.isGeneratingGraph) {
            ...
            toastr.info('已停止生成图谱', "Verification插件");
        }
    });
    ...
    $("#graph-file-upload").off("change").on("change", (e) => {
        ...
        reader.onload = (event) => {
            try {
                ...
                toastr.success('知识图谱导入完成！', "Verification插件");
            } catch (error) {
                toastr.error(`导入失败：${error.message}`, "Verification插件");
            } finally {
    ...
    $("#graph-copy-btn").off("click").on("click", async () => {
        ...
        if (!graphText) {
            toastr.warning('没有可复制的图谱内容', "Verification插件");
            return;
        }
        const success = await copyToClipboard(graphText);
        success ? toastr.success('图谱JSON已复制到剪贴板', "Verification插件") : toastr.error('复制失败', "Verification插件");
    });
    $("#graph-export-btn").off("click").on("click", () => {
        const graphText = $('#merged-graph-preview').val();
        if (!graphText) {
            toastr.warning('没有可导出的图谱内容', "Verification插件");
            return;
        }
        ...
        toastr.success('图谱JSON已导出', "Verification插件");
    });
    $("#graph-clear-btn").off("click").on("click", () => {
        ...
        saveSettingsDebounced();
        toastr.success('已清空合并图谱', "Verification插件");
    });
    ...
    $("#precheck-run-btn").off("click").on("click", () => {
        ...
        const modifiedContent = $("#write-chapter-content").val().trim();
        if (!selectedId) {
            toastr.error('请先选择基准章节', "Verification插件");
            return;
        }
    ...
    $("#write-stop-btn").off("click").on("click", () => {
        if (state.isGeneratingWrite) {
            ...
            toastr.info('已停止生成续写内容', "Verification插件");
        }
    });
    $("#write-copy-btn").off("click").on("click", async () => {
        const writeText = $('#write-content-preview').val();
        if (!writeText) {
            toastr.warning('没有可复制的续写内容', "Verification插件");
            return;
        }
        const success = await copyToClipboard(writeText);
        success ? toastr.success('续写内容已复制到剪贴板', "Verification插件") : toastr.error('复制失败', "Verification插件");
    });
    $("#write-send-btn").off("click").on("click", () => {
        ...
        if (!writeText) {
            toastr.warning('没有可发送的续写内容', "Verification插件");
            return;
        }
        if (!currentCharName) {
            toastr.error('请先选择一个聊天角色', "Verification插件");
            return;
        }
        ...
        context.executeSlashCommandsWithOptions(command).then(() => {
            toastr.success('续写内容已发送到对话框', "Verification插件");
        }).catch((error) => {
            toastr.error(`发送失败: ${error.message}`, "Verification插件");
        });
    });
    $("#write-clear-btn").off("click").on("click", () => {
        ...
        extension_settings[extensionName].qualityResultShow = false;
        saveSettingsDebounced();
        toastr.success('已清空续写内容', "Verification插件");
    });
    $("#clear-chain-btn").off("click").on("click", () => {
        ...
        renderContinueWriteChain(state.continueWriteChain);
        NovelReader.renderChapterList();
        toastr.success('已清空所有续写章节', "Verification插件");
    });

    console.log('[Verification] 全局事件绑定完成');
}

// 主初始化函数，导出给index.js调用
export async function mainInit() {
    try {
        ...
        console.error('[Verification] 主初始化报错', error);
        throw error;
    }
}
