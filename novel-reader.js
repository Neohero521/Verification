...
export const NovelReader = {
    ...
    renderChapterList() {
        ...
        const totalChapterCount = state.currentParsedChapters.length + state.continueWriteChain.length;
        ...
        state.currentParsedChapters.forEach(chapter => {
            const continueChapters = state.continueWriteChain.filter(item => item.baseChapterId === chapter.id);
    ...
    loadChapter(chapterId, chapterType = "original") {
        ...
        if (chapterType === "original") {
            chapterData = state.currentParsedChapters.find(item => item.id === chapterId);
            if (!chapterData) {
                this.resetAllLocks();
                return;
            }
            chapterTitle = chapterData.title;
            chapterIndex = state.currentParsedChapters.findIndex(item => item.id === chapterId) + 1;
        } else {
            chapterData = state.continueWriteChain.find(item => item.id === chapterId);
            if (!chapterData) {
                this.resetAllLocks();
                return;
            }
            const baseChapter = state.currentParsedChapters.find(item => item.id === chapterData.baseChapterId);
            const continueIndex = state.continueWriteChain.filter(item => item.baseChapterId === chapterData.baseChapterId).findIndex(item => item.id === chapterId) + 1;
            chapterTitle = `${baseChapter?.title || '未知章节'} - 续写章节 ${continueIndex}`;
            chapterIndex = state.currentParsedChapters.length + state.continueWriteChain.findIndex(item => item.id === chapterId) + 1;
        }
    ...
    loadNextChapter() {
        ...
        if (this.currentChapterType === "original") {
            const currentIndex = state.currentParsedChapters.findIndex(item => item.id === this.currentChapterId);
            if (currentIndex < 0 || currentIndex >= state.currentParsedChapters.length - 1) {
                this.resetAllLocks();
                return;
            }
            nextChapterId = state.currentParsedChapters[currentIndex + 1].id;
            nextChapterType = "original";
        } else {
            const currentChapter = state.continueWriteChain.find(item => item.id === this.currentChapterId);
            if (!currentChapter) {
                this.resetAllLocks();
                return;
            }
            const sameBaseChapters = state.continueWriteChain.filter(item => item.baseChapterId === currentChapter.baseChapterId);
            ...
            if (sameBaseIndex >= 0 && sameBaseIndex < sameBaseChapters.length - 1) {
                ...
                const baseChapterIndex = state.currentParsedChapters.findIndex(item => item.id === currentChapter.baseChapterId);
                if (baseChapterIndex < 0 || baseChapterIndex >= state.currentParsedChapters.length - 1) {
                    this.resetAllLocks();
                    return;
                }
                nextChapterId = state.currentParsedChapters[baseChapterIndex + 1].id;
                nextChapterType = "original";
            }
        }
    ...
    loadPrevChapter() {
        ...
        if (this.currentChapterType === "original") {
            ...
            prevChapterId = state.currentParsedChapters[currentIndex - 1].id;
