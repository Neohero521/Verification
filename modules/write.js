import { extensionName, qualityEvaluateSchema } from "./constants.js";
import { state } from "./state.js";
import { setButtonDisabled, copyToClipboard, renderCommandTemplate } from "./utils.js";
import { getActivePresetParams } from "./preset.js";
import { generateSingleChapterGraph, updateGraphWithContinueContent } from "./graph.js";
import { renderChapterSelect } from "./chapter.js";
...
export async function validateContinuePrecondition(baseChapterId, modifiedChapterContent = null) {
    ...
    try {
        const result = await generateRaw({ systemPrompt, prompt: userPrompt, jsonSchema: { name: 'ContinuePrecheck', strict: true, value: { type: "object", required: ["isPass", "preMergedGraph", "人设红线清单", "设定禁区清单", "可呼应伏笔清单", "潜在矛盾预警", "可推进剧情方向", "合规性报告"], properties: { isPass: { type: "boolean"}, preMergedGraph: { type: "object"}, "人设红线清单": { type: "string"}, "设定禁区清单": { type: "string"}, "可呼应伏笔清单": { type: "string"}, "潜在矛盾预警": { type: "string"}, "可推进剧情方向": { type: "string"}, "合规性报告": { type: "string"} } } }, ...getActivePresetParams()});
...
export async function evaluateContinueQuality(continueContent, precheckResult, baseGraph, baseChapterContent, targetWordCount) {
    ...
    try {
        const result = await generateRaw({ systemPrompt, prompt: userPrompt, jsonSchema: qualityEvaluateSchema, ...getActivePresetParams()});
...
export async function generateContinueWrite(targetChainId) {
    ...
    const preBaseChapters = state.currentParsedChapters.filter(chapter => chapter.id < baseChapterId);
    preBaseChapters.forEach(chapter => {
        fullContextContent += `${chapter.title}\n${chapter.content}\n\n`;
    });
    const baseChapterTitle = state.currentParsedChapters.find(c => c.id === baseChapterId)?.title || '基准章节';
    fullContextContent += `${baseChapterTitle}\n${editedBaseChapterContent}\n\n`;
    const targetBeforeChapters = state.continueWriteChain.slice(0, targetChainId + 1);
